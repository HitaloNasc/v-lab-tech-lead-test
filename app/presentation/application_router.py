...
from app.infrastructure.repositories.application_repository_sqlalchemy import ApplicationRepositorySQLAlchemy
from app.infrastructure.repositories.offer_repository_sqlalchemy import OfferRepositorySQLAlchemy
from app.infrastructure.repositories.candidate_profile_repository_sqlalchemy import CandidateProfileRepositorySQLAlchemy
...
async def create_application(app_in: ApplicationCreate, app_repo: ApplicationRepositorySQLAlchemy = Depends(get_application_repo), offer_repo: OfferRepositorySQLAlchemy = Depends(get_offer_repo), profile_repo: CandidateProfileRepositorySQLAlchemy = Depends(get_profile_repo)):
...
async def get_application(application_id: UUID, repo: ApplicationRepositorySQLAlchemy = Depends(get_application_repo)):
    use_case = GetApplicationById(repo)
    application = await use_case.execute(application_id)
    if not application:
        raise NotFoundError(message="Application not found", details=[{"field": "id", "reason": "not found"}])
    return ApplicationRead.from_domain(application)


@router.get("/by-candidate/{candidate_profile_id}", response_model=List[ApplicationRead])
async def list_by_candidate(candidate_profile_id: UUID, limit: int = 20, offset: int = 0, repo: ApplicationRepositorySQLAlchemy = Depends(get_application_repo)):
...
async def update_application(application_id: UUID, app_in: ApplicationUpdate, repo: ApplicationRepositorySQLAlchemy = Depends(get_application_repo)):
...
async def delete_application(application_id: UUID, deleted_by: UUID, repo: ApplicationRepositorySQLAlchemy = Depends(get_application_repo)):
