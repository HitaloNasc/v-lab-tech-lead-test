...
async def create_profile(profile_in: CandidateProfileCreate, candidate_repo: CandidateProfileRepositorySQLAlchemy = Depends(get_candidate_repo), user_repo: UserRepositorySQLAlchemy = Depends(get_user_repo)):
    use_case = CreateCandidateProfile(candidate_repo, user_repo)
    profile = await use_case.execute(profile_in.user_id, profile_in.full_name, profile_in.date_of_birth, profile_in.cpf)
    return CandidateProfileRead.from_domain(profile)


@router.get("/{profile_id}", response_model=CandidateProfileRead)
async def get_profile(profile_id: UUID, repo: CandidateProfileRepositorySQLAlchemy = Depends(get_candidate_repo)):
...
async def get_profile_by_user(user_id: UUID, repo: CandidateProfileRepositorySQLAlchemy = Depends(get_candidate_repo)):
    use_case = GetCandidateProfileByUserId(repo)
    profile = await use_case.execute(user_id)
    if not profile:
        raise NotFoundError(message="CandidateProfile not found", details=[{"field": "user_id", "reason": "not found"}])
    return CandidateProfileRead.from_domain(profile)


@router.put("/{profile_id}", response_model=CandidateProfileRead)
async def update_profile(profile_id: UUID, profile_in: CandidateProfileUpdate, repo: CandidateProfileRepositorySQLAlchemy = Depends(get_candidate_repo)):
...
async def delete_profile(profile_id: UUID, deleted_by: UUID, repo: CandidateProfileRepositorySQLAlchemy = Depends(get_candidate_repo)):
