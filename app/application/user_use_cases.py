...
class CreateUser:
    ...
    async def execute(self, email: str, password: str, full_name: Optional[str] = None, roles: Optional[list] = None, institution_id: Optional[UUID] = None) -> User:
        # hash password
        ...
        role_names = [getattr(r, "name", str(r)).lower() for r in roles_objs] if roles_objs else []
        if "admin" in role_names and not institution_id:
            raise ValidationError(message="admin users must have institution_id", details=[{"field": "institution_id", "reason": "required for admin role"}])
        if "candidate" in role_names and institution_id is not None:
            raise ValidationError(message="candidate users cannot be linked to an institution", details=[{"field": "institution_id", "reason": "must be null for candidate role"}])
        ...
        user = User(email=email, hashed_password=hashed, full_name=full_name, roles=roles_objs, institution_id=institution_id)
...
class UpdateUser:
    ...
    async def execute(self, user: User) -> User:
        ...
        role_names = [getattr(r, "name", str(r)).lower() for r in new_roles] if new_roles else []
        inst_id = getattr(user, "institution_id", getattr(current, "institution_id", None))
