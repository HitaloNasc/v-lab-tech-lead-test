...
class CreateApplication:
    def __init__(self, repo: ApplicationRepository, offer_repo: OfferRepository, profile_repo: CandidateProfileRepository):
        self.repo = repo
    ...
    async def execute(self, candidate_profile_id: UUID, offer_id: UUID) -> Application:
        ...
        if not offer:
            raise NotFoundError(message="Offer not found", details=[{"field": "offer_id", "reason": "not found"}])
        # check deadline
        if hasattr(offer, "application_deadline") and offer.application_deadline <= datetime.utcnow():
            raise ValidationError(message="Offer expired or deadline passed", details=[{"field": "offer_id", "reason": "expired"}])
        # validate candidate profile exists
        profile = await self.profile_repo.get_by_id(candidate_profile_id)
        if not profile:
            raise NotFoundError(message="CandidateProfile not found", details=[{"field": "candidate_profile_id", "reason": "not found"}])
        # uniqueness check
        existing = await self.repo.get_by_candidate_and_offer(candidate_profile_id, offer_id)
        if existing:
            raise ConflictError(message="Application already exists", details=[{"field": "candidate_profile_id,offer_id", "reason": "duplicate"}])

        application = Application(candidate_profile_id=candidate_profile_id, offer_id=offer_id)
...
class ListApplicationsByCandidate:
    ...
    async def execute(self, candidate_profile_id: UUID, limit: int = 20, offset: int = 0) -> List[Application]:
        return await self.repo.list_by_candidate_profile(candidate_profile_id, limit=limit, offset=offset)
